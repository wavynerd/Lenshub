...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		add_action( 'lisfinity_custom_fields_import', array( $this, 'import_custom_fields_and_taxonomies' ) );
	}
	...
	public function admin_page() {
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'lisfinity' ),
		);
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return __( 'Please define default parameters in the form of an array.', 'lisfinity' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return __( 'Please define an SVG icon filename.', 'lisfinity' );
		}
	...
	public function steps() {
		...
		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Welcome', 'lisfinity' ),
		...
		$this->steps['info'] = [
			'name' => esc_html__( 'info', 'lisfinity' ),
			'view' => [ $this, 'lisfinity_info' ],
		];

		$this->steps['child'] = array(
			'name' => esc_html__( 'Child', 'lisfinity' ),
			'view' => array( $this, 'child' ),
		);

		if ( $this->license_step_enabled ) {
			$this->steps['license'] = array(
				'name' => esc_html__( 'License', 'lisfinity' ),
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'lisfinity' ),
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'lisfinity' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'lisfinity' ),
	...
	protected function welcome() {
		...
		<?php
		$this->logger->debug( __( 'The welcome step has been displayed', 'lisfinity' ) );
	}
	...
	protected function license() {
		...
		<?php
		$this->logger->debug( __( 'The license activation step has been displayed', 'lisfinity' ) );
	}
	...
	protected function lisfinity_info() {
		...
		<div class="merlin__content--transition">
			...
			<h1><?php esc_html_e( 'Server Requirements', 'lisfinity-core' ); ?></h1>

			<p id="child-theme-text"><?php esc_html_e( 'Values are just a recommendation and not a strict requirement. But in the end, if the demo installation fails then please make sure that all values are met.', 'lisfinity-core' ); ?></p>

			<?php if ( ! empty( $checks ) ) : ?>
				<div class="lisfinity--step-info merlin__drawer--open">
					<div class="merlin__drawer merlin__drawer--install-plugins lisfinity-info--content">
						<div class="lisfinity-step--headers">
							<div class="header--first"></div>
							<div class="header--second"><?php esc_html_e( 'Suggested', 'lisfinity' ); ?></div>
							<div class="header--third"><?php esc_html_e( 'Your Server', 'lisfinity' ); ?></div>
						</div>
						<div class="lisfinity-step--content">
					...
					<a class="merlin__button merlin__button--knockout merlin__button--no-chevron merlin__button--external"
					   href="<?php echo esc_url( 'https://pebas.gitbook.io/lisfinity-documentation/overview/readme/theme-requirements' ); ?>"
					   target="_blank"><?php esc_html_e( 'Learn how to increase values', 'lisfinity-core' ); ?></a>
				</div>
			<?php endif; ?>
		...
		<?php
		$this->logger->debug( __( 'The server info installation step has been displayed', 'lisfinity' ) );
	}
	...
	protected function child() {
		...
		<?php
		$this->logger->debug( __( 'The child theme installation step has been displayed', 'lisfinity' ) );
	}
	...
	protected function plugins() {
						...
						<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
							<li data-slug="<?php echo esc_attr( $slug ); ?>">
								...
								<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
									...
									<span class="badge">
									<span class="hint--top"
										  aria-label="<?php esc_html_e( 'Required', 'lisfinity' ); ?>">
										<?php esc_html_e( 'req', 'lisfinity' ); ?>
									</span>
								</span>
								</label>
							</li>
		...
		<?php
		$this->logger->debug( __( 'The plugin installation step has been displayed', 'lisfinity' ) );
	}
	...
	protected function content() {
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				<div class="merlin__select-control-wrapper">
					...
					<div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'lisfinity' ); ?>">
			...
			<?php foreach ( $this->import_files as $index => $import_file ) : ?>
				<figure class="demo--preview <?php echo 0 === $index ? esc_attr( 'active' ) : ''; ?>"
					...
					<img
						...
						alt="<?php esc_attr_e( 'Demo Preview', 'lisfinity-core' ); ?>">
				</figure>
			<?php endforeach; ?>
		...
		<?php
		$this->logger->debug( __( 'The content import step has been displayed', 'lisfinity' ) );
	}
	...
	protected function ready() {
		...
		do_action( 'lisfinity_demo_completed' );
		?>
		...
		<?php
		$this->logger->debug( __( 'The final step has been displayed', 'lisfinity' ) );
	}
	...
	public function generate_child() {
		...
		if ( ! file_exists( $path ) ) {
			...
			$this->logger->debug( __( 'The existing child theme was activated', 'lisfinity' ) );
		...
		$this->logger->debug( __( 'The newly generated child theme was activated', 'lisfinity' ) );
	...
	public function _ajax_activate_license() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'lisfinity' ),
				)
			);
		}
		...
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'lisfinity' ),
				)
			);
		}
		...
		$this->logger->debug( __( 'The license activation was performed with the following results', 'lisfinity' ), $result );
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'lisfinity' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						...
						/* translators: Expiration date */
							esc_html__( 'Your license key expired on %s.', 'lisfinity' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'lisfinity' );
						break;

					case 'missing':
						$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'lisfinity' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'lisfinity' );
						break;

					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'lisfinity' ), $this->edd_item_name );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'lisfinity' );
						break;
	...
	public function generate_child_functions_php( $slug ) {
		...
		$this->logger->debug( __( 'The child theme functions.php content was generated', 'lisfinity' ) );
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$this->logger->debug( __( 'The child theme style.css content was generated', 'lisfinity' ) );
	...
	public function generate_child_screenshot( $path ) {
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			$this->logger->debug( __( 'The child theme screenshot was copied to the child theme, with the following result', 'lisfinity' ), array( 'copied' => $copied ) );
		} else {
			$this->logger->debug( __( 'The child theme screenshot was not generated, because of these results', 'lisfinity' ), array( 'screenshot' => $screenshot ) );
		}
	}
	...
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'lisfinity' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'lisfinity' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'lisfinity' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			$this->logger->debug(
				__( 'A plugin with the following data will be processed', 'lisfinity' ),
				array(
			...
			$json['message'] = esc_html__( 'Installing', 'lisfinity' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
				__( 'A plugin with the following data was processed', 'lisfinity' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
			...
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'lisfinity' ),
				)
			);
		}
	...
	function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			$this->logger->error( __( 'The content importer AJAX call failed to start, because of incorrect data', 'lisfinity' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'lisfinity' ),
				)
			);
		}
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				$this->logger->info(
					__( 'The content import AJAX call will be executed with this import data', 'lisfinity' ),
					array(
		...
		if ( $json ) {
			...
			$this->logger->error(
				__( 'The content import AJAX call failed with this passed data', 'lisfinity' ),
				array(
			...
			wp_send_json(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'lisfinity' ),
					'logs'    => '',
					'errors'  => '',
				)
			);
	...
	public function _ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			$this->logger->error( __( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'lisfinity' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'lisfinity' ),
				)
			);
		}
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title'            => esc_html__( 'Content', 'lisfinity' ),
				'description'      => esc_html__( 'Demo content data.', 'lisfinity' ),
				'pending'          => esc_html__( 'Pending', 'lisfinity' ),
				'installing'       => esc_html__( 'Installing', 'lisfinity' ),
				'success'          => esc_html__( 'Success', 'lisfinity' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'lisfinity' ),
				'description'      => esc_html__( 'Sample widgets data.', 'lisfinity' ),
				'pending'          => esc_html__( 'Pending', 'lisfinity' ),
				'installing'       => esc_html__( 'Installing', 'lisfinity' ),
				'success'          => esc_html__( 'Success', 'lisfinity' ),
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'lisfinity' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'lisfinity' ),
				'pending'          => esc_html__( 'Pending', 'lisfinity' ),
				'installing'       => esc_html__( 'Installing', 'lisfinity' ),
				'success'          => esc_html__( 'Success', 'lisfinity' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'lisfinity' ),
				'description'      => esc_html__( 'Sample theme options data.', 'lisfinity' ),
				'pending'          => esc_html__( 'Pending', 'lisfinity' ),
				'installing'       => esc_html__( 'Installing', 'lisfinity' ),
				'success'          => esc_html__( 'Success', 'lisfinity' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'lisfinity' ),
				'description'      => esc_html__( 'Redux framework options.', 'lisfinity' ),
				'pending'          => esc_html__( 'Pending', 'lisfinity' ),
				'installing'       => esc_html__( 'Installing', 'lisfinity' ),
				'success'          => esc_html__( 'Success', 'lisfinity' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'lisfinity' ),
				'description'      => esc_html__( 'After import setup.', 'lisfinity' ),
				'pending'          => esc_html__( 'Pending', 'lisfinity' ),
				'installing'       => esc_html__( 'Installing', 'lisfinity' ),
				'success'          => esc_html__( 'Success', 'lisfinity' ),
	...
	public function import_revolution_sliders( $file ) {
		...
		$this->logger->info( __( 'The revolution slider import was executed', 'lisfinity' ) );
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Installing', 'lisfinity' );
		$data['proceed']  = 'true';
	...
	public function after_content_import_setup() {
		...
		lisfinity_update_listing_expiring_dates();
	...
	public function import_custom_fields_and_taxonomies() {
		$selected_index = intval( $_POST['selected_index'] );
		if ( 'classic' === $this->import_files[ $selected_index ]['type'] ) {
			ob_start();
			require get_parent_theme_file_path() . '/includes/demo/classic/groups.json'; // to add a file with the fields.
			$group_fields = ob_get_clean();
			ob_start();
			require get_parent_theme_file_path() . '/includes/demo/classic/builder.json'; // to add a file with the fields.
			$search_builder_fields = ob_get_clean();
		}
		if ( 'boats' === $this->import_files[ $selected_index ]['type'] ) {
			ob_start();
			require get_parent_theme_file_path() . '/includes/demo/boats/groups.json'; // to add a file with the fields.
			$group_fields = ob_get_clean();

			ob_start();
			require get_parent_theme_file_path() . '/includes/demo/boats/builder.json'; // to add a file with the fields.
			$search_builder_fields = ob_get_clean();
		}
		if ( 'cars' === $this->import_files[ $selected_index ]['type'] ) {
			ob_start();
			require get_parent_theme_file_path() . '/includes/demo/cars/groups.json'; // to add a file with the fields.
			$group_fields = ob_get_clean();

			ob_start();
			require get_parent_theme_file_path() . '/includes/demo/cars/builder.json'; // to add a file with the fields.
			$search_builder_fields = ob_get_clean();
		}
		if ( 'realties' === $this->import_files[ $selected_index ]['type'] ) {
			ob_start();
			require get_parent_theme_file_path() . '/includes/demo/realties/groups.json'; // to add a file with the fields.
			$group_fields = ob_get_clean();

			ob_start();
			require get_parent_theme_file_path() . '/includes/demo/realties/builder.json'; // to add a file with the fields.
			$search_builder_fields = ob_get_clean();
		}
		if ( 'pets' === $this->import_files[ $selected_index ]['type'] ) {
			ob_start();
			require get_parent_theme_file_path() . '/includes/demo/pets/groups.json'; // to add a file with the fields.
			$group_fields = ob_get_clean();

			ob_start();
			require get_parent_theme_file_path() . '/includes/demo/pets/builder.json'; // to add a file with the fields.
			$search_builder_fields = ob_get_clean();
		}
		if ( 'small' === $this->import_files[ $selected_index ]['type'] ) {
			ob_start();
			require get_parent_theme_file_path() . '/includes/demo/small/groups.json'; // to add a file with the fields.
			$group_fields = ob_get_clean();
			ob_start();
			require get_parent_theme_file_path() . '/includes/demo/small/builder.json'; // to add a file with the fields.
			$search_builder_fields = ob_get_clean();
		}

		// import group fields and taxonomies.
		if ( ! empty( $group_fields ) ) {
			lisfinity_import_fields_demo( $group_fields );
			if ( class_exists( 'Lisfinity_Core' ) ) {
				$taxonomies_model = new \Lisfinity\Models\Taxonomies\TaxonomiesAdminModel();
				$taxonomies_model->register_new_taxonomies();
			}

		}
		if ( ! empty( $search_builder_fields ) ) {
			lisfinity_import_search_fields_demo( $search_builder_fields );
		}

		if ( ! empty( $options ) ) {
			lisfinity_import_options_demo( $options );
		}
	}
	...
	public function validate_import_file_info( $import_files ) {
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				$this->logger->warning( __( 'This predefined demo import does not have the name parameter: import_file_name', 'lisfinity' ), $import_file );
			}
		}
