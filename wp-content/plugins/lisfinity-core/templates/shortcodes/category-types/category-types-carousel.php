...
<?php if ( ! empty( $args['categories'] ) ) : ?>
	...
	<div
		class="category-types relative flex-nowrap -mb-30 xl:px-44 justify-center overflow-x-hidden category-types--slider" data-category-types-number="<?php echo esc_attr( $args['settings']['category_items_number_desktop'] ); ?>">
		<?php $count = 0; ?>
		<?php foreach ( $args['categories'] as $group => $taxonomies ) : ?>
			...
			<div
				...
				<div class="category-group--inner relative h-full rounded overflow-hidden hover:shadow-xl ?>">
					...
					<?php
					$url = add_query_arg( 'category-type', $group, $search_page_permalink );
					...
					<?php if ( ! empty( $image ) ) : ?>
						<figure class="category-group--bg absolute w-full h-full">
							<?php if ( isset( $args['settings']['background_image_overlay_taxonomies'] ) && 'yes' === $args['settings']['background_image_overlay_taxonomies'] ) : ?>
					...
					<?php $taxonomy = $args['settings']["taxonomy[{$group}]"] ?? ''; ?>
					<?php $terms = []; ?>
