<?php
/**
 * Template Name: Shortcodes | Pricing Packages | Subscriptions
			...
			<div class="flex justify-center w-full mx-auto px-col sm:px-0">
				...
				<div
					...
					<?php while ( $packages->have_posts() ) : ?>
						...
						<?php $style = carbon_get_post_meta( $product_id, 's-package-style' ); ?>
						<?php $package_footnotes = []; ?>
						<?php $discounts = carbon_get_post_meta( $product_id, 'subscription-discounts' ); ?>
						<?php if ( 'yes' === carbon_get_theme_option( 'multicurrency-enabled' ) && ! empty( $discounts ) ): ?>
							<?php foreach ( $discounts as $index => $discount ) : ?>
								<?php $discounts[ $index ]['discount'] = (float) $discount['discount'] * lisfinity_get_chosen_currency_rate(); ?>
							<?php endforeach; ?>
						<?php endif; ?>
						...
						<div
							...
							<?php $sale_price = $wc_product->get_sale_price(); ?>
							<?php $price = $wc_product->get_price(); ?>

							<!-- Price Package | Qty -->
							<div class="flex items-center mt-20 -mb-20">
								<label for="<?php echo esc_attr( "qty-$product_id" ); ?>" class="font-light text-sm text-grey-900"><?php esc_html_e( 'Choose duration', 'lisfinity-core' ); ?></label>
								<div class="mx-8 p-4 bg-white border border-grey-300 rounded" style="width: 54px;">
									<input id="<?php echo esc_attr( "qty-$product_id" ); ?>" type="number" min="1" class="w-full bg-transparent" value="1">
								</div>
								<span id="qty-label" class="font-light text-sm text-grey-900" data-singular="<?php esc_attr_e( 'Month', 'lisfinity-core' ); ?>" data-plural="<?php esc_attr_e( 'Months', 'lisfinity-core' ); ?>"><?php esc_html_e( 'Month', 'lisfinity-core' ); ?></span>
							</div>

							<!-- Price Package | Price -->
							<div
								class="package--price relative flex mt-10 font-bold text-5xl text-grey-1100 whitespace-nowrap">
								<?php if ( '2' == $style ) : ?>
									<span
										data-discounts="<?php echo esc_attr( json_encode( $discounts ) ); ?>"
										data-input="<?php echo esc_attr( "qty-$product_id" ); ?>"
										data-price="<?php echo esc_attr( $wc_product->get_price() * lisfinity_get_chosen_currency_rate() ); ?>"
								...
								<span
									...
									data-input="<?php echo esc_attr( "qty-$product_id" ); ?>"
									data-price="<?php echo esc_attr( $wc_product->get_price() * lisfinity_get_chosen_currency_rate() ); ?>"
									class="relative z-10 <?php echo ! empty( $sale_price ) ? esc_attr( 'on-sale' ) : ''; ?>"><?php echo wp_kses_post( lisfinity_get_price_html( $wc_product->get_price() * lisfinity_get_chosen_currency_rate() ) ); ?></span>
							</div>
							<div id="<?php echo esc_attr( "discount--qty-$product_id" ); ?>" class="hidden text-sm text-red-600"><?php _e( 'You get a discount of: ' ) ?><span class="discount"><?php echo esc_html( '0' ); ?></span></div>

							<?php $features = carbon_get_post_meta( $product_id, 's-package-features' ); ?>
							<?php if ( ! empty( $features ) ) : ?>
								<!-- Price Package | Features -->
								<ul class="package--features flex flex-col mt-30">
									<?php foreach ( $features as $feature ) : ?>
										<li class="relative mb-8">
											<?php _e( lisfinity_convert_to_option( $feature['package-feature'], $product_id ), 'lisfinity-core' ); ?>
							...
							<?php if ( is_user_logged_in() ) : ?>
								<button type="button"
										...
										data-discount="<?php echo esc_attr( $wc_product->get_price() ); ?>"
