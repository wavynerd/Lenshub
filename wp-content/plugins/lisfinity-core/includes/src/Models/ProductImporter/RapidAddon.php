    ...
    <h3 class="hndle" style="margin-top:0;"><span>' . $acf_group['title'] . '</span></h3>
		...
		function render_field( $field_params, $field_slug, $current_values, $in_the_bottom = false ) {
			...
			if ( $field_params['type'] == 'text' ) {
				...
				?>
				<h4 class="wpallimport-add-on-options-title"><?php _e( $field_params['name'], 'lisfinity-core' ); ?><?php if ( ! empty( $field_params['tooltip'] ) ): ?>
			...
			if ( ! get_option( sanitize_key( $this->slug ) . '_notice_ignore' ) ) {
				...
				<div class="error notice is-dismissible wpallimport-dismissible" style="margin-top: 10px;"
					...
					<p><?php _e(
							sprintf(
								$notice_text,
								'lisfinity-core' . $this->slug . 'lisfinity-core'
							),
